--- ### neovim Status
--
-- This module is automatically loaded by neovim on during it's initialization into global variable `neovim.status`
--
...
neovim.status = { hl = {}, init = {}, provider = {}, condition = {}, component = {}, utils = {}, env = {} }

neovim.status.env.modes = {
  ["n"] = { "NORMAL", "normal" },
  ["no"] = { "OP", "normal" },
  ["nov"] = { "OP", "normal" },
  ["noV"] = { "OP", "normal" },
...
neovim.status.env.icon_highlights =  {
  file_icon = {
...
neovim.status.env.buf_matchers = {
...
function neovim.status.hl.lualine_mode(mode, fallback)
...
-- @usage local heirline_component = { provider = "Example Provider", hl = neovim.status.hl.mode },
function neovim.status.hl.mode() return { bg = neovim.status.hl.mode_bg() } end
...
-- @usage local heirline_component = require("heirline.utils").surround({ "|", "|" }, neovim.status.hl.mode_bg, heirline_component),

function neovim.status.hl.mode_bg() return neovim.status.env.modes[vim.fn.mode()][2] end

--- Get the foreground color group for the current filetype
-- @return the highlight group for the current filetype foreground
-- @usage local heirline_component = { provider = neovim.status.provider.fileicon(), hl = neovim.status.hl.filetype_color },
function neovim.status.hl.filetype_color(self)
...
-- @usage local heirline_component = { provider = "Example Provider", hl = neovim.status.hl.get_attributes("treesitter") },
function neovim.status.hl.get_attributes(name, include_bg)
  local hl = neovim.status.env.attributes[name] or {}
  hl.fg = name .. "_fg"
...
-- @usage local heirline_component = { provider = "Example Provider", hl = neovim.status.hl.file_icon("winbar") },
function neovim.status.hl.file_icon(name)
  return function(self)
    local hl_enabled = neovim.status.env.icon_highlights.file_icon[name]
    if type(hl_enabled) == "function" then hl_enabled = hl_enabled(self) end
    if hl_enabled then return neovim.status.hl.filetype_color(self) end
  end
end
...
-- @usage local heirline_component = { init = neovim.status.init.breadcrumbs { padding = { left = 1 } } }
function neovim.status.init.breadcrumbs(opts)
  opts = neovim.default_tbl(opts, {
    separator = " > ",
    icon = { enabled = true, hl = neovim.status.env.icon_highlights.breadcrumbs },
  ...
  return function(self)
    ...
    for i, d in ipairs(data) do
      local pos = neovim.status.utils.encode_pos(d.lnum, d.col, self.winnr)
      local child = {
        { provider = string.gsub(d.name, "%%", "%%%%"):gsub("%s*->%s*", "") }, -- add symbol name
        on_click = { -- add on click function
          minwid = pos,
          callback = function(_, minwid)
            local lnum, col, winnr = neovim.status.utils.decode_pos(minwid)
    ...
    if opts.padding.left > 0 then -- add left padding
      table.insert(children, 1, { provider = neovim.pad_string(" ", { left = opts.padding.left - 1 }) })
    end
    if opts.padding.right > 0 then -- add right padding
      table.insert(children, { provider = neovim.pad_string(" ", { right = opts.padding.right - 1 }) })
    end
...
-- @return The Heirline init function
-- @usage local heirline_component = { init = neovim.status.init.update_events { "BufEnter", { "User", pattern = "LspProgressUpdate" } } }
function neovim.status.init.update_events(opts)
  return function(self)
...
-- @usage local heirline_component = { provider = neovim.status.provider.fill }
function neovim.status.provider.fill() return "%=" end

--- A provider function for the current tab numbre
-- @return the statusline function to return a string for a tab number
-- @usage local heirline_component = { provider = neovim.status.provider.tabnr() }
function neovim.status.provider.tabnr()
...
-- @usage local heirline_component = { provider = neovim.status.provider.spell() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.spell(opts)
  opts = neovim.default_tbl(opts, { str = "", icon = { kind = "Spellcheck" }, show_empty = true })
  return function() return neovim.status.utils.stylize(vim.wo.spell and opts.str, opts) end
end
...
-- @usage local heirline_component = { provider = neovim.status.provider.paste() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.paste(opts)
  opts = neovim.default_tbl(opts, { str = "", icon = { kind = "Paste" }, show_empty = true })
  return function() return neovim.status.utils.stylize(vim.opt.paste:get() and opts.str, opts) end
end
...
-- @usage local heirline_component = { provider = neovim.status.provider.macro_recording() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.macro_recording(opts)
  opts = neovim.default_tbl(opts, { prefix = "@" })
  return function()
    local register = vim.fn.reg_recording()
    if register ~= "" then register = opts.prefix .. register end
    return neovim.status.utils.stylize(register, opts)
  end
end
...
-- @usage local heirline_component = { provider = neovim.status.provider.search_count() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.search_count(opts)
  ...
  return function()
    ...
    if search_ok and type(search) == "table" and search.total then
      return neovim.status.utils.stylize(
        string.format(
          "%s%d/%s%d",
...
-- @usage local heirline_component = { provider = neovim.status.provider.mode_text() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.mode_text(opts)
  local max_length =
    math.max(unpack(vim.tbl_map(function(str) return #str[1] end, vim.tbl_values(neovim.status.env.modes))))
  return function()
    local text = neovim.status.env.modes[vim.fn.mode()][1]
    ...
    return neovim.status.utils.stylize(text, opts)
  end
end
...
-- @usage local heirline_component = { provider = neovim.status.provider.percentage() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.percentage(opts)
  opts = neovim.default_tbl(opts, { fixed_width = false, edge_text = true })
  return function()
...
-- @usage local heirline_component = { provider = neovim.status.provider.ruler({ pad_ruler = { line = 3, char = 2 } }) }
-- @see neovim.status.utils.stylize
function neovim.status.provider.ruler(opts)
  opts = neovim.default_tbl(opts, { pad_ruler = { line = 0, char = 0 } })
  local padding_str = string.format("%%%dd:%%%dd", opts.pad_ruler.line, opts.pad_ruler.char)
  return function()
    local line = vim.fn.line "."
    local char = vim.fn.virtcol "."
    return neovim.status.utils.stylize(string.format(padding_str, line, char), opts)
  end
end
...
-- @usage local heirline_component = { provider = neovim.status.provider.scrollbar() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.scrollbar(opts)
  local sbar = { "▁", "▂", "▃", "▄", "▅", "▆", "▇", "█" }
  return function()
    local curr_line = vim.api.nvim_win_get_cursor(0)[1]
    local lines = vim.api.nvim_buf_line_count(0)
    local i = math.floor((curr_line - 1) / lines * #sbar) + 1
    return neovim.status.utils.stylize(string.rep(sbar[i], 2), opts)
  end
end
...
-- @usage local heirline_component = { provider = neovim.status.provider.close_button() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.close_button(opts)
  opts = neovim.default_tbl(opts, { kind = "BufferClose" })
  return neovim.status.utils.stylize(neovim.get_icon(opts.kind), opts)
end

--- A provider function for showing the current filetype
-- @param opts options passed to the stylize function
-- @return the function for outputting the filetype
-- @usage local heirline_component = { provider = neovim.status.provider.filetype() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.filetype(opts)
  return function(self)
    local buffer = vim.bo[self and self.bufnr or 0]
    return neovim.status.utils.stylize(string.lower(buffer.filetype), opts)
  end
end
...
-- @usage local heirline_component = { provider = neovim.status.provider.filename() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.filename(opts)
  opts = neovim.default_tbl(
    opts,
  ...
  return function(self)
    ...
    return neovim.status.utils.stylize((filename == "" and opts.fallback or filename), opts)
  end
end
...
-- @usage local heirline_component = { provider = neovim.status.provider.unique_path() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.unique_path(opts)
  opts = neovim.default_tbl(opts, {
  ...
  return function(self)
    ...
    for _, value in ipairs(vim.g.heirline_bufferline and vim.t.bufs or neovim.status.utils.get_valid_buffers()) do
    ...
    return neovim.status.utils.stylize(
      (
        ...
        and string.sub(unique_path, 1, opts.max_length - 2) .. neovim.get_icon "Ellipsis" .. "/"
      ) or unique_path,
      opts
    )
  end
...
-- @usage local heirline_component = { provider = neovim.status.provider.file_modified() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.file_modified(opts)
  opts = neovim.default_tbl(opts, { str = "", icon = { kind = "FileModified" }, show_empty = true })
  return function(self)
    return neovim.status.utils.stylize(
      neovim.status.condition.file_modified((self or {}).bufnr) and opts.str,
      opts
    )
  end
end
...
-- @usage local heirline_component = { provider = neovim.status.provider.file_read_only() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.file_read_only(opts)
  opts = neovim.default_tbl(opts, { str = "", icon = { kind = "FileReadOnly" }, show_empty = true })
  return function(self)
    return neovim.status.utils.stylize(
      neovim.status.condition.file_read_only((self or {}).bufnr) and opts.str,
      opts
    )
  end
end
...
-- @usage local heirline_component = { provider = neovim.status.provider.file_icon() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.file_icon(opts)
  return function(self)
    local devicons_avail, devicons = pcall(require, "nvim-web-devicons")
    if not devicons_avail then return "" end
    ...
    return neovim.status.utils.stylize(ft_icon, opts)
  end
end
...
-- @usage local heirline_component = { provider = neovim.status.provider.git_branch() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.git_branch(opts)
  return function(self) return neovim.status.utils.stylize(vim.b[self and self.bufnr or 0].gitsigns_head or "", opts) end
end
...
-- @usage local heirline_component = { provider = neovim.status.provider.git_diff({ type = "added" }) }
-- @see neovim.status.utils.stylize
function neovim.status.provider.git_diff(opts)
...
-- @usage local heirline_component = { provider = neovim.status.provider.diagnostics({ severity = "ERROR" }) }
-- @see neovim.status.utils.stylize
function neovim.status.provider.diagnostics(opts)
  ...
  return function(self)
    ...
    return neovim.status.utils.stylize(count ~= 0 and tostring(count) or "", opts)
  end
end
...
-- @usage local heirline_component = { provider = neovim.status.provider.lsp_progress() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.lsp_progress(opts)
  return function()
...
-- @usage local heirline_component = { provider = neovim.status.provider.lsp_client_names({ expand_null_ls = true, truncate = 0.25 }) }
-- @see neovim.status.utils.stylize
function neovim.status.provider.lsp_client_names(opts)
  opts = neovim.default_tbl(opts, { expand_null_ls = true, truncate = 0.25 })
  return function(self)
    ...
    for _, client in pairs(vim.lsp.get_active_clients { bufnr = self and self.bufnr or 0 }) do
      if client.name == "null-ls" and opts.expand_null_ls then
        ...
        for _, type in ipairs { "FORMATTING", "DIAGNOSTICS" } do
          for _, source in ipairs(neovim.null_ls_sources(vim.bo.filetype, type)) do
    ...
    if type(opts.truncate) == "number" then
      local max_width = math.floor(neovim.status.utils.width() * opts.truncate)
      if #str > max_width then str = string.sub(str, 0, max_width) .. "…" end
    end
    return neovim.status.utils.stylize(str, opts)
  end
end
...
-- @usage local heirline_component = { provider = neovim.status.provider.treesitter_status() }
-- @see neovim.status.utils.stylize
function neovim.status.provider.treesitter_status(opts)
  return function()
    return neovim.status.utils.stylize(require("nvim-treesitter.parser").has_parser() and "TS" or "", opts)
  end
end
...
-- @usage local heirline_component = { provider = neovim.status.provider.str({ str = "Hello" }) }
-- @see neovim.status.utils.stylize
function neovim.status.provider.str(opts)
  opts = neovim.default_tbl(opts, { str = " " })
  return neovim.status.utils.stylize(opts.str, opts)
end

--- A condition function if the window is currently active
-- @return boolean of wether or not the window is currently actie
-- @usage local heirline_component = { provider = "Example Provider", condition = neovim.status.condition.is_active }
function neovim.status.condition.is_active() return vim.api.nvim_get_current_win() == tonumber(vim.g.actual_curwin) end
...
-- @usage local heirline_component = { provider = "Example Provider", condition = function() return neovim.status.condition.buffer_matches { buftype = { "terminal" } } end }
function neovim.status.condition.buffer_matches(patterns, bufnr)
  for kind, pattern_list in pairs(patterns) do
    if neovim.status.env.buf_matchers[kind](pattern_list, bufnr) then return true end
  end
  return false
end
...
-- @usage local heirline_component = { provider = "Example Provider", condition = neovim.status.condition.is_macro_recording }
function neovim.status.condition.is_macro_recording() return vim.fn.reg_recording() ~= "" end

--- A condition function if search is visible
-- @return boolean of wether or not searching is currently visible
-- @usage local heirline_component = { provider = "Example Provider", condition = neovim.status.condition.is_hlsearch }
function neovim.status.condition.is_hlsearch() return vim.v.hlsearch ~= 0 end
...
-- @usage local heirline_component = { provider = "Example Provider", condition = neovim.status.condition.is_git_repo }
function neovim.status.condition.is_git_repo(bufnr)
...
-- @usage local heirline_component = { provider = "Example Provider", condition = neovim.status.condition.git_changed }
function neovim.status.condition.git_changed(bufnr)
  if type(bufnr) == "table" then bufnr = bufnr.bufnr end
...
-- @usage local heirline_component = { provider = "Example Provider", condition = neovim.status.condition.file_modified }
function neovim.status.condition.file_modified(bufnr)
  if type(bufnr) == "table" then bufnr = bufnr.bufnr end
...
-- @usage local heirline_component = { provider = "Example Provider", condition = neovim.status.condition.file_read_only }
function neovim.status.condition.file_read_only(bufnr)
  if type(bufnr) == "table" then bufnr = bufnr.bufnr end
...
-- @usage local heirline_component = { provider = "Example Provider", condition = neovim.status.condition.has_diagnostics }
function neovim.status.condition.has_diagnostics(bufnr)
  if type(bufnr) == "table" then bufnr = bufnr.bufnr end
...
-- @usage local heirline_component = { provider = "Example Provider", condition = neovim.status.condition.has_filetype }
function neovim.status.condition.has_filetype(bufnr)
  if type(bufnr) == "table" then bufnr = bufnr.bufnr end
...
-- @usage local heirline_component = { provider = "Example Provider", condition = neovim.status.condition.aerial_available }
-- function neovim.status.condition.aerial_available() return neovim.is_available "aerial.nvim" end
function neovim.status.condition.aerial_available() return package.loaded["aerial"] end

--- A condition function if LSP is attached
-- @param bufnr a buffer number to check the condition for, a table with bufnr property, or nil to get the current buffer
-- @return boolean of wether or not LSP is attached
-- @usage local heirline_component = { provider = "Example Provider", condition = neovim.status.condition.lsp_attached }
function neovim.status.condition.lsp_attached(bufnr)
  if type(bufnr) == "table" then bufnr = bufnr.bufnr end
...
-- @usage local heirline_component = { provider = "Example Provider", condition = neovim.status.condition.treesitter_available }
function neovim.status.condition.treesitter_available(bufnr)
...
-- @usage local string = neovim.status.utils.stylize("Hello", { padding = { left = 1, right = 1 }, icon = { kind = "String" } })
function neovim.status.utils.stylize(str, opts)
  opts = neovim.default_tbl(opts, {
    padding = { left = 0, right = 0 },
  ...
  local icon = neovim.pad_string(neovim.get_default_icon(opts.icon.kind), opts.icon.padding)
  return str
      and (str ~= "" or opts.show_empty)
      and opts.separator.left .. neovim.pad_string(icon .. str, opts.padding) .. opts.separator.right
    or ""
end
...
-- @usage local heirline_component = neovim.status.component.fill()
function neovim.status.component.fill(opts)
  return neovim.default_tbl(opts, { provider = neovim.status.provider.fill() })
end
...
-- @return The Heirline component table
-- @usage local heirline_component = neovim.status.component.file_info()
function neovim.status.component.file_info(opts)
  opts = neovim.default_tbl(opts, {
    file_icon = {
      hl = neovim.status.hl.file_icon "statusline",
      padding = { left = 1, right = 1 },
    }, -- TODO: REWORK THIS
    filename = {},
    file_modified = { padding = { left = 1 } },
    file_read_only = { padding = { left = 1 } },
    surround = { separator = "left", color = "file_info_bg", condition = neovim.status.condition.has_filetype },
    hl = neovim.status.hl.get_attributes "file_info",
  })
  return neovim.status.component.builder(neovim.status.utils.setup_providers(opts, {
    "file_icon",
    "unique_path",
    "filename",
    "filetype",
...
-- @param opts options for configuring file_icon, filename, filetype, file_modified, file_read_only, and the overall padding
-- @return The Heirline component table
-- @usage local heirline_component = neovim.status.component.tabline_file_info()
function neovim.status.component.tabline_file_info(opts)
  return neovim.status.component.file_info(neovim.default_tbl(opts, {
    file_icon = {
      condition = function(self) return not self._show_picker end,
      hl = neovim.status.hl.file_icon "tabline",
    },
    unique_path = {
      hl = function(self) return neovim.status.hl.get_attributes(self.tab_type .. "_path") end,
    },
    close_button = {
      hl = function(self) return neovim.status.hl.get_attributes(self.tab_type .. "_close") end,
      padding = { left = 1, right = 1 },
      on_click = {
        callback = function(_, minwid) neovim.close_buf(minwid) end,
    ...
    hl = function(self)
      ...
      return neovim.status.hl.get_attributes(tab_type)
    end,
    surround = false,
  }))
end
...
-- @return The Heirline component table
-- @usage local heirline_component = neovim.status.component.nav()
function neovim.status.component.nav(opts)
  opts = neovim.default_tbl(opts, {
    ruler = {},
    percentage = { padding = { left = 1 } },
    scrollbar = { padding = { left = 1 }, hl = { fg = "scrollbar" } },
    surround = { separator = "right", color = "nav_bg" },
    hl = neovim.status.hl.get_attributes "nav",
    update = { "CursorMoved", "BufEnter" },
  })
  return neovim.status.component.builder(
    neovim.status.utils.setup_providers(opts, { "ruler", "percentage", "scrollbar" })
  )
end
...
-- @return The Heirline component table
-- @usage local heirline_component = neovim.status.component.macro_recording()
-- TODO: deprecate on next major version release
function neovim.status.component.macro_recording(opts)
  opts = neovim.default_tbl(opts, {
    macro_recording = { icon = { kind = "MacroRecording", padding = { right = 1 } } },
    surround = {
      separator = "center",
      color = "macro_recording_bg",
      condition = neovim.status.condition.is_macro_recording,
    },
    hl = neovim.status.hl.get_attributes "macro_recording",
    update = { "RecordingEnter", "RecordingLeave" },
  })
  return neovim.status.component.builder(neovim.status.utils.setup_providers(opts, { "macro_recording" }))
end
...
-- @return The Heirline component table
-- @usage local heirline_component = neovim.status.component.cmd_info()
function neovim.status.component.cmd_info(opts)
  opts = neovim.default_tbl(opts, {
    macro_recording = {
    ...
    search_count = {
      ...
      padding = { left = 1 },
      condition = neovim.status.condition.is_hlsearch,
    },
    surround = {
      separator = "center",
      color = "cmd_info_bg",
      condition = function()
        return neovim.status.condition.is_hlsearch() or neovim.status.condition.is_macro_recording()
      end,
    },
    condition = function() return vim.opt.cmdheight:get() == 0 end,
    hl = neovim.status.hl.get_attributes "cmd_info",
  })
  return neovim.status.component.builder(
    neovim.status.utils.setup_providers(opts, { "macro_recording", "search_count" })
  )
end
...
-- @return The Heirline component table
-- @usage local heirline_component = neovim.status.component.mode { mode_text = true }
function neovim.status.component.mode(opts)
  opts = neovim.default_tbl(opts, {
    mode_text = false,
    paste = false,
    spell = false,
    surround = { separator = "left", color = neovim.status.hl.mode_bg },
    hl = neovim.status.hl.get_attributes "mode",
    update = "ModeChanged",
  })
  if not opts["mode_text"] then opts.str = { str = " " } end
  return neovim.status.component.builder(
    neovim.status.utils.setup_providers(opts, { "mode_text", "str", "paste", "spell" })
  )
end
...
-- @return The Heirline component table
-- @usage local heirline_component = neovim.status.component.breadcumbs()
function neovim.status.component.breadcrumbs(opts)
  opts = neovim.default_tbl(
    opts,
    { padding = { left = 1 }, condition = neovim.status.condition.aerial_available, update = "CursorMoved" }
  )
  opts.init = neovim.status.init.breadcrumbs(opts)
  return opts
end
...
-- @return The Heirline component table
-- @usage local heirline_component = neovim.status.component.git_branch()
function neovim.status.component.git_branch(opts)
  opts = neovim.default_tbl(opts, {
    git_branch = { icon = { kind = "GitBranch", padding = { right = 1 } } },
    surround = { separator = "left", color = "git_branch_bg", condition = neovim.status.condition.is_git_repo },
    hl = neovim.status.hl.get_attributes "git_branch",
    on_click = {
    ...
    init = neovim.status.init.update_events { "BufEnter" },
  })
  return neovim.status.component.builder(neovim.status.utils.setup_providers(opts, { "git_branch" }))
end
...
-- @return The Heirline component table
-- @usage local heirline_component = neovim.status.component.git_diff()
function neovim.status.component.git_diff(opts)
  opts = neovim.default_tbl(opts, {
    ...
    hl = neovim.status.hl.get_attributes "git_diff",
    on_click = {
      name = "heirline_git",
      callback = function()
        if neovim.is_available "telescope.nvim" then
    ...
    surround = { separator = "left", color = "git_diff_bg", condition = neovim.status.condition.git_changed },
    update = { "User", pattern = "GitSignsUpdate" },
    init = neovim.status.init.update_events { "BufEnter" },
  })
  return neovim.status.component.builder(
    neovim.status.utils.setup_providers(opts, { "added", "changed", "removed" }, function(p_opts, provider)
      local out = neovim.status.utils.build_provider(p_opts, provider)
      if out then
...
-- @return The Heirline component table
-- @usage local heirline_component = neovim.status.component.diagnostics()
function neovim.status.component.diagnostics(opts)
  opts = neovim.default_tbl(opts, {
    ...
    surround = { separator = "left", color = "diagnostics_bg", condition = neovim.status.condition.has_diagnostics },
    hl = neovim.status.hl.get_attributes "diagnostics",
    on_click = {
  ...
  return neovim.status.component.builder(
    neovim.status.utils.setup_providers(opts, { "ERROR", "WARN", "INFO", "HINT" }, function(p_opts, provider)
      local out = neovim.status.utils.build_provider(p_opts, provider)
      if out then
...
-- @param opts options for configuring diagnostic providers and the overall padding
-- @return The Heirline component table
-- @usage local heirline_component = neovim.status.component.treesitter()
function neovim.status.component.treesitter(opts)
  opts = neovim.default_tbl(opts, {
    str = { str = "TS", icon = { kind = "ActiveTS" } },
    surround = {
      separator = "right",
      color = "treesitter_bg",
      condition = neovim.status.condition.treesitter_available,
    },
    hl = neovim.status.hl.get_attributes "treesitter",
    update = { "OptionSet", pattern = "syntax" },
    init = neovim.status.init.update_events { "BufEnter" },
  })
  return neovim.status.component.builder(neovim.status.utils.setup_providers(opts, { "str" }))
end
...
-- @return The Heirline component table
-- @usage local heirline_component = neovim.status.component.lsp()
function neovim.status.component.lsp(opts)
  opts = neovim.default_tbl(opts, {
    lsp_progress = {
      str = "",
      padding = { right = 1 },
    ...
    hl = neovim.status.hl.get_attributes "lsp",
    surround = { separator = "right", color = "lsp_bg", condition = neovim.status.condition.lsp_attached },
    on_click = {
      name = "heirline_lsp",
      callback = function()
  ...
  return neovim.status.component.builder(
    neovim.status.utils.setup_providers(
      opts,
      ...
      function(p_opts, provider, i)
        return p_opts
            and {
              flexible = i,
              neovim.status.utils.build_provider(p_opts, neovim.status.provider[provider](p_opts)),
              neovim.status.utils.build_provider(p_opts, neovim.status.provider.str(p_opts)),
            }
          or false
      end
    )
...
--- A general function to build a section of neovim status providers with highlights, conditions, and section surrounding
-- @param opts a list of components to build into a section
-- @return The Heirline component table
-- @usage local heirline_component = neovim.status.components.builder({ { provider = "file_icon", opts = { padding = { right = 1 } } }, { provider = "filename" } })
function neovim.status.component.builder(opts)
  opts = neovim.default_tbl(opts, { padding = { left = 0, right = 0 } })
  local children = {}
  if opts.padding.left > 0 then -- add left padding
    table.insert(children, { provider = neovim.pad_string(" ", { left = opts.padding.left - 1 }) })
  end
  for key, entry in pairs(opts) do
    if
      type(key) == "number"
      and type(entry) == "table"
      and neovim.status.provider[entry.provider]
      and (entry.opts == nil or type(entry.opts) == "table")
    then
      entry.provider = neovim.status.provider[entry.provider](entry.opts)
    end
    children[key] = entry
  end
  if opts.padding.right > 0 then -- add right padding
  ...
  return opts.surround
      and neovim.status.utils.surround(
...
-- @param provider a provider in `neovim.status.providers`
-- @return the provider table that can be used in `neovim.status.component.builder`
function neovim.status.utils.build_provider(opts, provider, _)
  return opts
      and {
        provider = provider,
        opts = opts,
...
-- @param setup a function that takes provider options table, provider name, provider index and returns the setup provider table, optional, default is `neovim.status.utils.build_provider`
-- @return the fully setup options table with the appropriately ordered providers
function neovim.status.utils.setup_providers(opts, providers, setup)
  setup = setup or neovim.status.utils.build_provider
...
function neovim.status.utils.width(is_winbar)
...
-- @param separator the separator index to use in `neovim.status.env.separators`
...
function neovim.status.utils.surround(separator, color, component, condition)
  ...
  separator = type(separator) == "string" and neovim.status.env.separators[separator] or separator
...
function neovim.status.utils.is_valid_buffer(bufnr) -- TODO v3: remove this function
...
function neovim.status.utils.get_valid_buffers() -- TODO v3: remove this function
  return vim.tbl_filter(neovim.status.utils.is_valid_buffer, vim.api.nvim_list_bufs())
end
...
function neovim.status.utils.encode_pos(line, col, winnr)
